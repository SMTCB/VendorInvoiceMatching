...
async function runTest(invoiceNumber: string) {
    ...
    const extractedData = {
        ...
        total_amount: 12.00, // S3 Invoice total
        currency: invoice.currency,
        line_items: [{ desc: "Widget A", qty: 10, unit_price: 1.20 }], // $1.20 * 10 = $12.00. PO is $1.00 * 10 = $10.00
        text: "Invoice INV-S3 for PO 4500002003. Total $12.00."
    };

    console.log("Extracted Data:", JSON.stringify(extractedData, null, 2));

    // 2. Fetch PO Lines
    const poRef = (extractedData.po_reference || '').replace('PO', '').replace(/\s+/g, '').trim();
